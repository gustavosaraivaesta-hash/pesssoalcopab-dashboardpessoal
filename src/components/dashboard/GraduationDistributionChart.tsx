...
export const GraduationDistributionChart = ({ 
  ...
  const renderCustomLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percentual }: any) => {
    const RADIAN = Math.PI / 180;
    const radius = innerRadius + (outerRadius - innerRadius) * 0.5;
    const x = cx + radius * Math.cos(-midAngle * RADIAN);
    const y = cy + radius * Math.sin(-midAngle * RADIAN);

    if (percentual < 3) return null; // Não mostrar label para fatias muito pequenas
        ...
        <ResponsiveContainer width="100%" height={400}>
          <PieChart>
            ...
            <Legend 
              ...
              height={36}
              formatter={(value, entry: any) => `${value}: ${entry.payload.quantidade} (${entry.payload.percentual}%)`}
            />
          </PieChart>
        </ResponsiveContainer>
      </CardContent>
